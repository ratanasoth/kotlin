...
fun Runnable.test(f: Runnable.(Int) -> Unit) {
    f(<!NI;TYPE_MISMATCH!><!NI;TYPE_MISMATCH!><!TYPE_MISMATCH!>""<!><!><!>)
}
...
fun Int.test(f: String.(Int) -> Unit) {
    ...
    with("") {
        f(0)
        f(<!NI;CONSTANT_EXPECTED_TYPE_MISMATCH!><!NI;CONSTANT_EXPECTED_TYPE_MISMATCH!><!CONSTANT_EXPECTED_TYPE_MISMATCH!>0.0<!><!><!>)
    }
}