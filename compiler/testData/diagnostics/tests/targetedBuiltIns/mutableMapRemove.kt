...
fun foo(x: MutableMap<String, Int>, y: java.util.HashMap<String, Int>, z: java.util.AbstractMap<String, Int>) {
    x.remove("", 1)
    x.remove("", <!NI;TYPE_MISMATCH!><!TYPE_MISMATCH!>""<!><!>)
    x.remove("", <!NI;NULL_FOR_NONNULL_TYPE!><!NULL_FOR_NONNULL_TYPE!>null<!><!>)

    y.remove("", 1)
    y.remove("", <!NI;TYPE_MISMATCH!><!TYPE_MISMATCH!>""<!><!>)
    y.remove("", <!NI;NULL_FOR_NONNULL_TYPE!><!NULL_FOR_NONNULL_TYPE!>null<!><!>)

    z.remove("", 1)
    z.remove("", <!NI;TYPE_MISMATCH!><!TYPE_MISMATCH!>""<!><!>)
    z.remove("", null)
}
